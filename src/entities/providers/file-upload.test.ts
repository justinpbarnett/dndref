...
Commands the east watch.`,
    ...
    expect(entities.map(({ name, type, aliases, summary }) => ({ name, type, aliases, summary }))).toEqual([
      ...
      { name: "Captain Aria", type: "NPC", aliases: ["Aria", "the captain"], summary: "Commands the east watch." },
      {
        name: "The Sundering Blade",
        type: "Item",
        aliases: ["Sundering Blade", "the blade"],
...
`,
    ...
    expect(entities.map(({ name, type, aliases, summary }) => ({ name, type, aliases, summary }))).toEqual([
      { name: "Lord Ember", type: "NPC", aliases: ["Ember"], summary: "Rules the cinder court." },
    ]);
  });
});
