...
describe("GoogleDocsProvider", () => {
  ...
  it("fetches the exported document text through the browser CORS proxy", async () => {
    ...
    await expect(fetchGoogleDocText("https://docs.google.com/document/d/doc_123/edit")).resolves.toBe(
      "# Moonlit Bazaar",
    );
...
`;
  ...
  it("surfaces failed public document exports through the provider failure path", async () => {
    ...
    await expect(new GoogleDocsProvider("doc_123").load()).rejects.toThrow("Google Docs fetch failed: 403");
  ...
  it("keeps browser CORS errors on the existing user-friendly path", async () => {
    ...
    await expect(fetchGoogleDocText("doc_123")).rejects.toThrow(
      "Cannot reach Google Docs from the browser (CORS). Use the iOS app or paste content via file upload.",
    );
  });
});
