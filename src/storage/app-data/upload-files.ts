...
const uploadMutationQueue = new AsyncOperationQueue();
...
export const waitForUploadedFileMutations = (): Promise<void> => uploadMutationQueue.wait();
...
function mutateUploadedFiles(mutator: (uploads: UploadedFile[]) => UploadedFile[]): Promise<boolean> {
  ...
  return uploadMutationQueue.run(async () => {
...
export const resetUploadedFileMutationQueueForTests = (): void => uploadMutationQueue.reset();
